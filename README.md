...
<p align="center">
    <a href="https://github.com/danwashusen/spec-kit-ext/actions/workflows/release.yml"><img src="https://github.com/danwashusen/spec-kit-ext/actions/workflows/release.yml/badge.svg" alt="Release"/></a>
...
This extension builds on the upstream toolkit to help teams with legacy estates and heavily regulated environments. Spec Kit Ext layers in:

- Richer context loading across `/speckit.*` commands so you can ground decisions in existing PRDs, ADRs, runbooks, and compliance checklists.
- Project scaffolding tuned for incremental modernization and integration, not just greenfield prototypes.
- Opinionated defaults that encourage documenting why systems exist today before reshaping them, keeping legacy constraints visible while you plan the future state.
...
| [Amp](https://ampcode.com/) | ✅ | Install the [`amp`](https://ampcode.com/manual#install) CLI before running slash commands |
...
| Command     | Description                                                    |
|-------------|----------------------------------------------------------------|
| `init`      | Initialize a new Spec Kit Ext project from the latest template |
| `check`     | Check for installed tools (`git`, `claude`, `gemini`, `code`/`code-insiders`, `cursor-agent`, `windsurf`, `qwen`, `opencode`, `codex`, `q`) |
...
specify-ext init my-project --ai cursor-agent

# Initialize with Windsurf support
specify-ext init my-project --ai windsurf

# Initialize with Amp support
specify-ext init my-project --ai amp
...
| `/speckit.audit`     | Run the governance-focused review playbook to enforce policy and control gates before release |
| `/speckit.changelog` | Prepare a detailed changelog entry and detect system documentation drift (after `/speckit.implement` or `/speckit.audit`) |
...
You can use the Specify Ext CLI to bootstrap your project, which will bring in the required artifacts in your environment. Run:
...
![Specify Ext CLI bootstrapping a new project in the terminal](./media/specify_cli.gif)
